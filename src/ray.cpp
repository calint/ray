...
public:
	...
	float dt(){
		...
		const long diff_us=tv.tv_usec-t0.tv_usec;
...
//		metrics::rayone=clk::timerdt();
	...
	void keydn(const unsigned char key,const int x,const int y){kb->onkeyb((signed char)key,true,x,y);}// players[gloxnet::player]->keydn((char)key,x,y);}
	void keyup(const unsigned char key,const int x,const int y){kb->onkeyb((signed char)key,false,x,y);}//players[gloxnet::player]->keyup((char)key,x,y);}
